import type { ContinuityIssue, StoryNode, WorldStateDelta } from "./models";
...
export function computeWorldState(nodesInOrder: StoryNode[]): WorldState {
...
export function computePreAndPostStateForNode(nodesInOrder: StoryNode[], nodeId: string): {
  pre: WorldState;
  post: WorldState;
  node: StoryNode;
} {
  ...
  for (const n of nodesInOrder) {
    if (n._id === nodeId) {
      ...
      return { pre, post, node: n };
    }
    state = applyDeltas(state, n.worldStateDelta ?? []);
  }

  throw new Error("Node not found in ordered list");
}

export function continuityCheckForNode(
  node: StoryNode,
  preState: WorldState,
  ...
  for (const p of node.participants ?? []) {
    ...
    const preLoc = getPath(preState, key);
    const postLoc = getPath(postState, key);
    ...
    if (sceneLocation && effectiveLoc && effectiveLoc !== sceneLocation) {
      issues.push({
        ...
        nodeId: node._id,
        entityIds: [p.entityId, sceneLocation],
        suggestion:
  ...
  // Basic item consistency: prevent resurrecting destroyed items
  for (const delta of node.worldStateDelta ?? []) {
    if (delta.key.startsWith("item.") && delta.key.endsWith(".status")) {
      const pre = getPath(preState, delta.key);
      if (pre === "DESTROYED" && delta.op === "SET" && delta.value !== "DESTROYED") {
        issues.push({
          severity: "ERROR",
          code: "ITEM_RESURRECTED",
          message: `Item status resurrected from DESTROYED to ${String(delta.value)}.`,
          nodeId: node._id,
          suggestion: "If this is intentional, introduce a new item entity or explain the recovery in-world.",
        });
      }
    }
  }
